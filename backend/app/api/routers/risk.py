...
from app.storage.db import get_latest_estimate_snapshot, list_estimate_snapshots, list_holdings
...
async def get_risk_overview(request: Request) -> dict:
    ...
    latest_estimate = get_latest_estimate_snapshot(snapshot_user_id)
    if not isinstance(latest_estimate, dict):
        if is_admin(request):
