    ...
    def test_fund_search_contract(self) -> None:
        with TestClient(app) as client:
            ...
            if search_items:
                ...
                self.assertIn("performance_ranges", detail_json)
                self.assertIn("integrity", detail_json)
                self.assertIn("anomalies", detail_json)
                ...
                ranges = detail_json.get("performance_ranges", {})
                self.assertIn("d7", ranges)
                self.assertIn("d30", ranges)
                self.assertIn("d90", ranges)
                self.assertIn("ytd", ranges)
                integrity = detail_json.get("integrity", {})
                self.assertIn("status", integrity)
                self.assertIn("recent_30d_coverage", integrity)
                self.assertTrue(isinstance(detail_json.get("anomalies", []), list))
