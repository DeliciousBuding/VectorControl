...
- [ ] [前端] 第二轮 bundle 优化：按 `openspec/changes/frontend-bundle-optimization-phase2/` 记录 `antd-vendor` 与页面级 chunk 基线，推进页面级懒加载与 bundle 收缩（已完成基线刷新：`index`≈101.51kB；交易中心循环分块告警已消除；SettingsDrawer、metrics 缓冲写入、首页重图表延后加载、非首页主路径组件 lazy 化、交易中心抽离为 `TradeCenter.jsx`、持仓新增与提醒规则抽离为 `HoldingsCreatePanel.jsx` / `ReminderRulesPanel.jsx`、`SettingsDrawer` 的 `Drawer width` 弃用噪音已清、`SettingsDrawer` 开发态 schema 测试噪音已清、`App.jsx` 未使用 Ant Design / icon 依赖清理、删除 `echartsCore.js` / `ReturnsChart.jsx` / `BenchmarkComparison.jsx` 并移除 `echarts` + `echarts-for-react` 依赖后，前端源码与 analyze 均已不再存在 ECharts runtime；当前新热点是 `antd-vendor`≈1006.91kB，下一步需继续压缩 vendor 体积）。
