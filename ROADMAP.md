...
- [√] [前端] **基金专属独立页面**：点击基金跳转独立路由 `/fund/:fund_id`，而非滚动到底部查看（已完成@bot）
- [√] [前端] **优化基金详情页信息密度**：左右布局（左波形图+右具体数据），波形图缩小提高信息密度（已完成@bot）
- [√] [前端] **持仓表头自定义字段**：支持显示/隐藏列，删除持仓份额为0的列（已完成@bot）
- [√] [后端] **导入交易记录JSON功能**：标准JSON格式+自动爬取补全缺失数据（已完成@bot a4cbd7b）
